package org.urbcomp.startdb.compress.camel.xorcompressor;
...
public class ElfXORCompressor32 {
    ...
    public ElfXORCompressor32() {
        out = new OutputBitStream(
                new byte[5000]);  // for elf, we need one more bit for each at the worst case
        size = 0;
    }
    ...
    private int writeFirst(int value) {
        ...
        if (trailingZeros < 32) {
            out.writeLong(storedVal >>> (trailingZeros + 1), 31 - trailingZeros);
        ...
        if (xor == 0) {
            ...
            if (leadingZeros == storedLeadingZeros && trailingZeros >= storedTrailingZeros) {
                ...
                if (centerBits <= 8) {
                    ...
                    out.writeInt(xor >>> (storedTrailingZeros + 1), centerBits - 1);
